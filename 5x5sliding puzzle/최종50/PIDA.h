...
namespace SSS55 {
	...
	void ida55initialization();
	int heuristic(int ttPuzzle[][MAXN]);
	bool valid(int r, int c);
	bool ThereIsRoute(int r, int ttPuzzle[][MAXN]);
	void IDA(int depth, int r, int c, int est, int pre);
	int getBlank();

	int ida55Limit = 0;      // 10000000 = > 1
	...
	void  ida55(int arr[5][5], int t)
	{
		...
		printf("\a");
		if (!ThereIsRoute(sr, ttPuzzle)) { canSolve = false; printf("\n#%d -1\n", T + 1); }
		else
		{
			canSolve = true;
			int cost = heuristic(ttPuzzle);
	...
	int getBlank() {
		...
		for (i = 0; i < MAXN * MAXN; i++)
			if (ttPuzzle[i / MAXN][i % MAXN] == MAXN * MAXN - 1)
				return i;
		return i;
	}
	...
	int heuristic(int ttPuzzle[][MAXN]) //맨해튼 거리를 heuristic function 으로 사용하였다.
	{
		int ret = 0;
		for (int i = 0; i < MAXN; i++)
			for (int j = 0; j < MAXN; j++)
				if (ttPuzzle[i][j] < MAXN * MAXN - 1)
					ret += abs(i - tr[ttPuzzle[i][j]]) + abs(j - tc[ttPuzzle[i][j]]);
		return ret;
	}
		...
		for (int i = 0; i < 4; i++) // 4가지 방향으로 이동한다.
			if (i != pre) //전에 이동했던 방향의 역방향으로는 이동하지 않는다.
			{
				...
				if (valid(nr, nc))
				{
					...
					{
						ttPuzzle[r][c] = bak, ttPuzzle[nr][nc] = MAXN * MAXN - 1;   // 바꾸기
						rec[depth] = name[i];//경로 저장
						IDA(depth + 1, nr, nc, est + nCost - oCost, opp[i]); // 재귀호출
						ttPuzzle[r][c] = MAXN * MAXN - 1, ttPuzzle[nr][nc] = bak; // 원래대로 바꾸기
